...
\paragraph{Runtime}
Both queries return the exact same result since ssnums are unique. The diference in runtime is due to the fact that conducting an index only scan on the unique index simply requires more block accesses than a sequential scan of the table.
