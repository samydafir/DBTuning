...
\end{verbatim}
\ \\
First, the optimizer uses an Index Scan on Employee, which has an Unique Index on ssnum. Afterwards the UNIQUE operator deletes all duplicates, but therefore it needs a sorted row which we got due to the Index Scan.
\\
\\
Execution plan rewritten query:
