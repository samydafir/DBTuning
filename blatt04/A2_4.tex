...
Show the runtime results and compute the throughput.\\
Runtime: 78877ms
Throughput: 19.01695044182715

\smallskip\noindent
Query plan (for one of the queries):
{\small
\begin{verbatim}
 Bitmap Heap Scan on publ_cb  (cost=45.98..6695.42 rows=2265 width=112) (actual time=1.183..6.727 rows=9182 loops=1)
   Recheck Cond: ((year)::text = '1986'::text)
   Heap Blocks: exact=160
   ->  Bitmap Index Scan on yearcb  (cost=0.00..45.41 rows=2265 width=0) (actual time=1.149..1.149 rows=9182 loops=1)
...
{\small
\begin{verbatim}
 Bitmap Heap Scan on publ_b  (cost=55.01..7010.65 rows=2398 width=112) (actual time=2.106..32.430 rows=9182 loops=1)
   Recheck Cond: ((year)::text = '1986'::text)
   Heap Blocks: exact=3258
   ->  Bitmap Index Scan on yearb  (cost=0.00..54.41 rows=2398 width=0) (actual time=1.604..1.604 rows=9182 loops=1)
         Index Cond: ((year)::text = '1986'::text)
 Planning time: 0.281 ms
 Execution time: 36.131 ms
\end{verbatim}
}
